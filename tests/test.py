...
INODE_ENTRY = construct.Struct("INODE_ENTRY",
        construct.UBInt64("fseg_id"),
...
def list_page(f):
    ...
    for p in page_fils:
        print(p.offset, p.pre_page, p.next_page, p.spaceid, PAGE_TYPE_MAP.get(p.page_type, "UNKNOW"))
...
def get_seek(list_base_node):
    return list_base_node.first_page_number * 1024 * 16 + list_base_node.first_page_offset, list_base_node.last_page_number * 1024 * 16 + list_base_node.last_page_offset
...
INDEX_HEADER = construct.Struct("INDEX_HEADER",
        construct.UBInt16("number_directory_slots"),
        construct.UBInt16("heap_top_position"), # 当前页中, 已经使用了的字节数, 之后的都是free
        construct.UBInt16("number_heap_records"), # 最高的bit存储, fromat flag, 当前页中records的格式, 可能的值: COMPACT/REDUNDANY
        # 其余存储, 本页的记录数: number(infimum) + num(supremum) + num(deleted)
        construct.UBInt16("first_garbage_record_offset"), # pointer 指向了garbage records列表的第一个entry
        construct.UBInt16("garbage_space"), # garbage records list 占用的字节辆
        construct.UBInt16("last_insert_position"), # 本page中最后一条records插入的位置
        construct.UBInt16("page_direction"), # LEFT, RIGHT, NO_DIRECTION, 表示当前页插入数据的时候, 插入位置与最后一条插入数据的位置关系

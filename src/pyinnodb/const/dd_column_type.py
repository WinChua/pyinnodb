...
class DDColConf(DDColConf, Enum):
    ...
    TINY = (
        ...
        1,
        int,
        lambda col: random.randint(0, 2**8 - 1)
        if col.is_unsigned
        else random.randint(-(2**7), 2**7 - 1),
    )
    SHORT = (
        DDColumnType.SHORT,
        2,
        int,
        lambda col: random.randint(0, 2**16 - 1)
        if col.is_unsigned
        else random.randint(-(2**15), 2**15 - 1),
    )
    LONG = (
        DDColumnType.LONG,
        4,
        int,
        lambda col: random.randint(0, 2**32 - 1)
        if col.is_unsigned
    ...
    TIMESTAMP = (
        DDColumnType.TIMESTAMP,
        0,
        int,
        lambda col: datetime.fromtimestamp(
            int(time.time()) + random.randint(-100, 100)
    ...
    LONGLONG = (
        ...
        8,
        int,
        lambda col: random.randint(0, 2**64 - 1)
        if col.is_unsigned
    ...
    INT24 = (
        ...
        3,
        int,
        lambda col: random.randint(0, 2**24 - 1)
        if col.is_unsigned
        else random.randint(-(2**23), 2**23 - 1),
    )
    DATE = (
        DDColumnType.DATE,
        0,
        date,
        lambda col: date(
            random.randint(0, 2000), random.randint(1, 12), random.randint(1, 28)
        ),
    )
    TIME = (
        DDColumnType.TIME,
        0,
        timedelta,
        lambda col: timedelta(random.randint(0, 100)),
    )
    DATETIME = (
        DDColumnType.DATETIME,
        0,
        datetime,
        lambda col: datetime(
    ...
    VARCHAR = (
        ...
        0,
        str,
        lambda col, size=None: random.randbytes(
            size if size is not None and size < col.varchar_size else col.varchar_size
        ).hex(),
    )
    BIT = DDColumnType.BIT, 0, int, lambda col: random.randint(0, 1)
    TIMESTAMP2 = (
        DDColumnType.TIMESTAMP2,
        0,
        int,
        lambda col: datetime.fromtimestamp(
            int(time.time()) + random.randint(-100, 100)
    ...
    TIME2 = (
        ...
        0,
        timedelta,
        lambda col: f"{random.randint(-838, 838)}:{random.randint(0, 59):02}:{random.randint(0, 59):02}.{random.randint(0, 30000)}",
    )
    ...
    ENUM = (
        ...
        0,
        str,
        lambda col: b64decode(random.choice(col.elements).name).decode(
            errors="replace"
        ),
    )
    SET = (

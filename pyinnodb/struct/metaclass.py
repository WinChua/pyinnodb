...
class Field(object):
    ...
    def __init__(self, parser, *args, **kwargs):
        ...
        self.create = Field.creation_counter
...
class MetaClass(type):
    def __new__(cls, name, bases, attrs):
        ...
        for f in fields:
            ...
            if isinstance(f.parser, MBits):
                bits_fields.append(f)
                continue
            elif len(bits_fields) > 0:
                bits_parser = construct.EmbeddedBitStruct(*[f.parser._make_con() for f in bits_fields])
                bits_names = ",".join([f.name for f in bits_fields])
                obj = CommonField(name=bits_names, parser=bits_parser, is_bit=True)
                bits_fields.clear()
        ...
        def _parse(self, stream, context=None):
            ...
            for field in self._parse_order:
                value = field.parser.parse_stream(stream)
                if getattr(field, "is_bit", None):
        ...
        def _build(self, obj, stream, context=None):
            for field in self._parse_order:
                field.parser.build_stream(getattr(obj, field.name), stream)

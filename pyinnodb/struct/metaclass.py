    ...
    def __new__(cls, name, bases, attrs):
        ...
        for f in fields:
            ...
            if isinstance(f.parser(f.name), MBits):
                ...
                bits_parser = construct.EmbeddedBitStruct(*[f.parser(f.name)._make_con() for f in bits_fields])
                bits_names = ",".join([f.name for f in bits_fields])
                obj = Field(CommonField(name=bits_names, parser=bits_parser, is_bit=True))
                obj.name = bits_names
                bits_fields.clear()
